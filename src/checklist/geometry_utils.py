...
# ---------------------------------------------------------------------------
# 7c. Outline-Boundary Crossing Detection
# ---------------------------------------------------------------------------

def _classify_segment(x1: float, y1: float,
                      x2: float, y2: float,
                      angle_threshold: float = 30.0) -> str:
    """Classify a line segment as 'Horizontal', 'Vertical', or 'Diagonal'.

    A segment is classified based on its angle relative to the axes.
    *angle_threshold* (in degrees) is the maximum deviation from an axis
    for the segment to be considered aligned with that axis.
    """
    dx = x2 - x1
    dy = y2 - y1
    ...
    if angle_deg < angle_threshold or angle_deg > (180.0 - angle_threshold):
        return "Horizontal"
    if abs(angle_deg - 90.0) < angle_threshold:
        return "Vertical"
    return "Diagonal"


def find_outline_crossing_components(
    comp: Component,
    candidates: Sequence[Component],
    packages: list[Package],
    ...
    boundary = outline.exterior  # LinearRing
    coords = list(boundary.coords)  # last == first for closed ring

    # Pre-build segment list: (LineString, classification)
    segments: list[tuple[LineString, str]] = []
    for i in range(len(coords) - 1):
        x1, y1 = coords[i]
        x2, y2 = coords[i + 1]
        seg_cls = _classify_segment(x1, y1, x2, y2)
        segments.append((LineString([(x1, y1), (x2, y2)]), seg_cls))

    result: list[tuple[Component, str]] = []

    for cand in candidates:
        fp = _resolve_footprint(cand, packages)
        if fp is None:
