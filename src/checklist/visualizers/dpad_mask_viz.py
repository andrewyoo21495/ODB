...
Produces one image per board side (Top/Bottom) showing the solder-mask
layer as the background, with all evaluated D-pad list capacitors drawn
over it — green for PASS, red for FAIL — and the Shield Can / Interposer
"inside regions" (convex hulls) drawn faintly for context.
...
def _filter_features(features: LayerFeatures, bbox) -> LayerFeatures:
    """Return a LayerFeatures with only features inside *bbox* (board coordinates)."""
...
def render_dpad_side_image(
    ...
    *,
    ...
    ----------
    cap_items : list[dict]
        One entry per evaluated capacitor. Each dict must have:
          - "cap":      Component
          - "status":   "PASS" or "FAIL"
    ...
    containers : list[Component]
        All Shield Cans / Interposers on this side (context overlay).
    mask_lf : LayerFeatures | None
