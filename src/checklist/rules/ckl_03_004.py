"""CKL-03-004: VIA presence check for Hall IC pads.

For Hall IC components (identified by part_name in references/hall_ic.csv),
every pad must have at least one VIA.  A pad with zero VIAs is flagged FAIL.
"""
...
class CKL03004(ChecklistRule):
    ...
    def evaluate(self, job_data: dict) -> RuleResult:
        ...
        hall_parts = get_managed_part_names("hall_ic")
        ...
        rows: list[dict] = []
        images: list[dict] = []
        image_dir = Path(tempfile.mkdtemp(prefix="ckl_03_004_"))
        ...
        for comps, layer_name, is_bottom in [
            ...
            hall_ics = [c for c in comps if (c.part_name or "") in hall_parts]

            for comp in hall_ics:
                ...
                # Generate visualisation image for this Hall IC
