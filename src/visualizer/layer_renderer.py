...
def _draw_pad(ax: Axes, pad: PadRecord, sym_lookup: dict[int, SymbolRef],
    ...
    flip_x    — mirrors the whole pad about x=0: negates x, rotation, mirror.
                Used in single-layer viewers (CopperRatioViewer, NetViewer) to
                show a bottom-side layer in "view from below" mode.
    flip_shape — flips the pad shape in-place without moving its board
                position: negates rotation and inverts mirror, keeps x.
                Used in PcbViewer for bottom-side layers whose orient_def is
                stored in bottom-view (component-side) convention.
    """
