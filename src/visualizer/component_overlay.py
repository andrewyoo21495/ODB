...
def _draw_component_geometry(ax: Axes, comp: Component,
    ...
    if draw_pads:
        for tp in comp.toeprints:
            ...
            geom = tp.geom
            # geom.x/y/rotation/mirror are in board space (top-view), sourced
            # directly from the ODB++ layer feature PadRecord.  No sign
            # inversion is needed for bottom components — the values are
            # already correct for the top-view rendering used by PcbViewer.
